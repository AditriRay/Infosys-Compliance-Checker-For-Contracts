...
def scrape_data(url, name):
    ...
    if response.status_code==200:
        ...
        notification.send_notification("Failed to Download File", 
            f"Response status is {response.status_code} and Download Link is {url}\nError is {response.text}")
...
def call_scrape_funtion():
    ...
    try:
        ...
        for key, value in DOCUMENT_MAP.items():
            # dealing with all agreements
            scrape_data(DOCUMENT_MAP[key]["link"], temp_agreement)
            
            clauses = data_extration.Clause_extraction(temp_agreement)
            
            # Load old template data
            old_data = None
            try:
                with open(DOCUMENT_MAP[key]["json_file"], "r", encoding="utf-8") as f:
                    old_data = json.load(f)
            except FileNotFoundError:
                print(f"No previous template found for {key}")
            ...
            if old_data is not None and old_data != clauses:
                notification.send_notification(
                    f"Template Update Detected - {key}", 
                    f"The template for {key} has been updated.\nOld clauses count: {len(json.loads(old_data)) if isinstance(old_data, str) else len(old_data)}\nNew clauses count: {len(json.loads(clauses))}\nPlease review the changes."
                )
